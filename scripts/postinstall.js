...
async function main() {
  console.log('');
  console.log(`${BOLD}BMAD Odoo installed!${RESET}`);
  console.log('');
  ...
  if (hasUpdateSystem) {
    ...
    if (scenario === 'fresh' || !currentVersion) {
      ...
      console.log('');
      console.log(`  ${CYAN}npx -p bmad-odoo bmad-install${RESET}  - Install all agents (${agentNames})`);
      console.log('');
      return;
    }
    ...
    if (currentVersion === targetVersion) {
      console.log(`${GREEN}✓ BMAD Odoo is up to date! (v${currentVersion})${RESET}`);
      console.log('');
      return;
    }
    ...
    if (compareVersions(currentVersion, targetVersion) < 0) {
      ...
      console.log(`  ${CYAN}npx -p bmad-odoo bmad-update --dry-run${RESET}`);
      console.log('');
      console.log('To apply the update:');
      console.log(`  ${CYAN}npx -p bmad-odoo bmad-update${RESET}`);
      console.log('');
