...
**Goal:** Run `bmad-audit-skill-dirs` against the current project and surface results.

**Your role:** Operator-tooling wrapper. Invoke the CLI, interpret exit code, and present findings clearly.

## Steps

1. Run `bmad-audit-skill-dirs --verbose` via Bash from the project root. Pipe stdout + stderr together so the operator sees everything.
...
**Distinction from `bmad-validate-marketplace`:** this audit covers the full `.claude/skills/` runtime tree (typically ~98 dirs). `validate-marketplace` only audits the 7 Vortex paths listed in `marketplace.json` — pre-submission gate, narrower scope.
